...
ob_start();
$token = "7932854395:AAE12OTEDARvfqe8le6VYCEb6q4kehc6DjE";
...
if ($data == 'VIP_button') {
    ...
    bot("EditMessageText", [
        "chat_id" => $chat_id, 
        'message_id' => $message_id,
        "text" => "[𝙱𝙻𝙰𝙲𝙺](tg://user?id=7790070110)\nمرحبا بك في قسم إدارة الـ VIP",
        'parse_mode' => "markdown",
        'reply_markup' => json_encode([
            'inline_keyboard' => [
...
function abdo() {
    ...
    return json_encode([
        'inline_keyboard' => [
            [['text' => 'تنبيه دخول الاعضاء  ' . $tak, 'callback_data' => "tak"]], // tm
...
if ($bot['bott'] != "on" and !in_array($from_id, $admins)) {
    ...
    bot($m, [
        ...
        'message_id' => $message_id,
        'text' => "
البوت حاليا في وضع الصيانه انتظر منا اشعارا اخر ❤
",
        'parse_mode' => "markdown",
    ]);
    exit;
...
$numberfiles = isset($bot["numberfiles"]) ? $bot["numberfiles"] : 7;
...
```⭐⭐⭐
انت من مستخدمي (vip) ```
لوحة التحكم الخاصه بك

يمكنك رفع الملفات عن طريق ارسالها هنا
الفولدر المحدد للرفع {$folder_id}
...
",
        ...
        'reply_markup' => json_encode([
            'inline_keyboard' => [
                [['text' => "🛠 - تحديث البوت ", 'callback_data' => "refr"], ['text' => "🎛 - تحديث الملفات  ", 'callback_data' => "nas"]],
                [['text' => "📨 - التواصل مع الدعم ", 'callback_data' => "contact"]],
                [['text' => "📑 - انشاء فولدر  ", 'callback_data' => "Create_folder"], ['text' => "🎬 - تعيين فولدر الرفع ", 'callback_data' => "set_flowr"]],
...
" ,
        ...
        if (preg_match('/(\d{6,14}:[\w-]{35,75})/', $f, $matches)) {
            ...
            if ($result != false) {
                ...
                s();
                $keyb = [
                    [['text'=>"• عمل ويبهوك •",'callback_data'=>"up_webhook|$cr" ],['text'=>"• حذف الويبهوك •",'callback_data'=>"del_webhook|$cr" ]],
